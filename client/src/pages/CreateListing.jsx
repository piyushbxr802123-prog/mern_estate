// CreateListing.jsx (Cloudinary version - NO Firebase Storage)
// - Upload up to 6 images to Cloudinary (unsigned preset)
// - Save listing to your MERN backend (/api/listing/create)
// Requirements:
// 1) Cloudinary: cloud name + unsigned upload preset
// 2) Backend: cookie auth (access_token) or adjust credentials
// 3) Vite env vars (recommended):
...
// optional limits (adjust if you want)
const MAX_IMAGES = 6;
...
export default function CreateListing() {
  ...
  const uploadToCloudinary = (file) => {
    return new Promise((resolve, reject) => {
      ...
      // data.append("folder", "mern-estate/listings"); // optional
  ...
  const handleImageSubmit = async () => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        // first image will be cover because it’s first in the array
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("/api/listing/create", {
        ...
        body: JSON.stringify({
          ...formData,
          userRef: currentUser?._id, // Mongo user id
        }),
      });
      ...
      // if your backend returns created listing with _id:
    ...
    <main className="p-3 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          {imageUploadError ? <p className="text-red-700 text-sm">{imageUploadError}</p> : null}
